...
class NewsRepository(
    ...
    suspend fun getHealthNews(countryCode: String, category: String, pageNumber: Int) =
        RetrofitInstance.api.getHealthNews(countryCode, category, pageNumber)
    ...
    suspend fun getNewsFromSpecificSource(source: String, pageNumber: Int) =
        RetrofitInstance.api.newsSources(source, pageNumber)

    suspend fun upsert(article: Article) = db.getArticleDao().upsert(article)

    fun getSavedNews() = db.getArticleDao().getAllArticle()

    suspend fun deleteAllArticles() = db.getArticleDao().deleteAllSavedArticles()
